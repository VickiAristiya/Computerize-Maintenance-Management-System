...
COMPONENT_LABELS = {
    "status": {"fault": "Fault", "ok": "Ok"},
}

COMPONENT_NAME_ID = {
    "status": "Spindle & Motor Mesin Bor",
}

_RISK_ORDER = ["very_low", "low", "medium", "high", "critical"]


class BorPredictor(BasePredictor):
    """Hybrid DNN-SVM predictor untuk mesin bor — status kelistrikan & getaran.

    Sensor mesin bor mengirim lebih banyak field daripada yang dipakai model
    (mis. energy_kwh, energy_wh, frequency_hz, voltage_v). Field di luar
    `feature_columns` sengaja tidak ikut masuk ke model — tetap tersimpan di
    SensorData/raw_readings untuk keperluan monitoring, tapi inferensi hanya
    memakai 6 sensor yang dipakai saat training.
    """
    ...
    def feature_columns(self):
        self._ensure_loaded()
        return list(self._bundles["status"]["feature_columns"])
    ...
    def _ensure_loaded(self):
        if not self.is_ready():
            raise FileNotFoundError(
                "Bor model files are missing: "
                + ", ".join(self.missing_model_files())
            )

        for component in COMPONENTS:
            if component not in BorPredictor._bundles:
                BorPredictor._bundles[component] = joblib.load(
                    self.models_dir / f"hybrid_model_{component}.pkl"
                )
            if component not in BorPredictor._extractors:
                BorPredictor._extractors[component] = tf.keras.models.load_model(
                    self.models_dir / f"dnn_extractor_{component}.keras"
                )

        self._bundles = BorPredictor._bundles
        self._extractors = BorPredictor._extractors
    ...
    def _build_aggregate_recommendation(self, faulty_components, components):
        if not faulty_components:
            return (
                "Mesin bor dalam kondisi baik (arus, daya, suhu, dan getaran normal). "
        ...
        return (
            "Mesin bor terindikasi fault — periksa kondisi spindle/bearing, "
            f"ketajaman mata bor, dan beban motor. {urgency}"
        )
