...
COMPONENT_LABELS = {
    "status": {"fault": "BoardFault", "ok": "Ok"},
}

COMPONENT_NAME_ID = {
    "status": "Papan Kontrol Induksi",
}

_RISK_ORDER = ["very_low", "low", "medium", "high", "critical"]


class InduksiPredictor(BasePredictor):
    """Hybrid DNN-SVM predictor untuk mesin induksi — status papan kontrol."""
    ...
    def _ensure_loaded(self):
        if not self.is_ready():
            raise FileNotFoundError(
                "Induksi model files are missing: "
                + ", ".join(self.missing_model_files())
            )

        for component in COMPONENTS:
            if component not in InduksiPredictor._bundles:
                InduksiPredictor._bundles[component] = joblib.load(
                    self.models_dir / f"hybrid_model_{component}.pkl"
                )
            if component not in InduksiPredictor._extractors:
                InduksiPredictor._extractors[component] = tf.keras.models.load_model(
                    self.models_dir / f"dnn_extractor_{component}.keras"
                )

        self._bundles = InduksiPredictor._bundles
        self._extractors = InduksiPredictor._extractors
    ...
    def _build_aggregate_recommendation(self, faulty_components, components):
        if not faulty_components:
            return (
                "Papan kontrol induksi dalam kondisi baik. "
        ...
        return f"Papan kontrol induksi terindikasi fault (BoardFault). {urgency}"
